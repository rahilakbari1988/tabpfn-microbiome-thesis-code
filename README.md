# Analyzing Microbiome Metabarcoding Data with Tabular Foundation Models
...
- 16S/18S amplicon profiles and physicochemical measurements: Sperlea et al.
  (2025), *Scientific Data* 12:1774, https://doi.org/10.1038/s41597-025-06249-1
- Anthropogenic trace substances: Vogel et al. (2025), *Environmental
  Pollution* 383:126916, https://doi.org/10.1016/j.envpol.2025.126916
  (dataset: https://doi.org/10.1594/PANGAEA.975100)

Place the merged source tables in `data/processed/` as `meta_master.csv`,
`otu_16s_master.csv` and `otu_18s_master.csv`. `notebooks/02_preprocessing.ipynb`
produces `meta_clean.csv`, `otu_16s_clean.csv` and `otu_18s_clean.csv`, which
all later notebooks read.

## Which code produces which result

| Thesis | Notebook / code | Result file |
|---|---|---|
...
| Chapter 6 — Phase 1 benchmark (TabPFN) | `notebooks/10_tabpfn_evaluation_strategies_v3.ipynb`, `src/tabpfn_evaluator.py` | `results/appendix_A_full72_all_metrics.csv` (rounded: `…_rounded.csv`) |
| Chapter 6 — classical baselines | `notebooks/05_lasso_…`, `06_xgboost_…`, `07_random_forest_…`; `src/*_evaluator.py` | `results/appendix_A_full72_all_metrics.csv` |
...
| Chapter 8 — Phase 3 forecasting | `notebooks/14_phase3_hplc.ipynb` | `results/appendix_B_forecasting_216.csv`, `results/chapter8_master_forecasting_table.csv`, `results/chapter8_forecasting_all_metrics_54cells.csv`, `results/chapter8_forecasting_headline_18cells.csv` |
...
(`/mnt/data/home/tv2414/tabpfn_project`). Adjust the `ROOT` / `PROJ` variable
at the top of each notebook to your own project directory before running. The
...
Akbari Fartkhouni, R. (2026). *Analyzing Microbiome Metabarcoding Data with
Tabular Foundation Models (TabPFN)*. MSc thesis, University of Rostock.

## Author

Rahil Akbari Fartkhouni, MSc Computer Science, University of Rostock.
